...
- Windows P0 使用原生文件/目录句柄、`FlushFileBuffers`、write-through no-replace/replace rename，以及 create/rename/verified delete 后的父目录刷新；逐 journal phase 的 Windows 硬中断矩阵仍属 P1，因此这些 recovery/lifecycle 证据不形成正式支持承诺，也不替代待决定的 fresh-deployment 策略；
...
Journal JSON, journal-directory contents, cleanup claims/markers, and journal creation/removal use the platform durability backend. macOS/Linux use file and directory `fsync`. Windows P0 uses native file/directory handles, `FlushFileBuffers`, write-through no-replace/replace renames, and parent-directory flushes after create, rename, and verified delete. The per-journal-phase Windows hard-interruption matrix remains P1, so this recovery/lifecycle evidence is not formal support and does not replace the pending fresh-deployment policy decision. Replacements created at the original path after a directory claim are preserved. The evidence model protects against accidental drift and ordinary races; it is not cryptographic authentication against a same-user process that coordinates edits to journal, intent, companion, manifest, or cleanup claims. Extreme power loss, devices that do not honor flush, filesystem/kernel corruption, or abnormal directory-entry persistence are also outside the provable boundary.
