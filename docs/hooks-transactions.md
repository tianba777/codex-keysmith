...
This document describes the v0.1.0 transaction behavior shipped by `codex-keysmith` plus the current durable-recovery fixes. It is the technical reference for deployment, interrupted deploy/uninstall recovery, hooks restore, status health, and manifest-based uninstall.
...
- Windows P0 使用原生文件/目录句柄、`FlushFileBuffers`、write-through no-replace/replace rename，以及 create/rename/verified delete 后的父目录刷新；逐 journal phase 的 Windows 硬中断矩阵仍属 P1，因此 Windows runtime 继续 unsupported，不形成正式支持承诺；
